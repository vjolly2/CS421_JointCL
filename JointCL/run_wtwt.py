...
from utils.data_utils import  Tokenizer4Bert, ZSSDDataset_vast
...
class Instructor(object):
    def __init__(self,opt):
        ...
        self.trainset = ZSSDDataset_vast(fname=self.opt.train_dir,tokenizer=tokenizer)
        self.valset = ZSSDDataset_vast(fname=self.opt.dev_dir, tokenizer=tokenizer)
        self.testset = ZSSDDataset_vast(fname=self.opt.test_dir,tokenizer=tokenizer)
    ...
    # compute results and return accuracy, f1 score for dev
    ...
    # Computate results and return accuracy, f1 score etc for test
    ...
    # Evaluation results are saved in a output file
...
# Sets the seed to reproduce the result
def set_seed(seed):
    random.seed(seed)
    np.random.seed(seed)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset', default='AET_HUM', type=str,required=False)
    ...
    parser.add_argument("--eval_steps", default=100, type=int, required=False)
    ...
    dataset_files = {
        'AET_HUM': {
            'train': './datasets/wtwt/o_AET_HUM',
            'test': './datasets/wtwt/AET_HUM',
        },
        'ANTM_CI': {
            'train': './datasets/wtwt/o_ANTM_CI',
            'test': './datasets/wtwt/ANTM_CI',
        },
        'CI_ESRX': {
            'train': './datasets/wtwt/o_CI_ESRX',
            'test': './datasets/wtwt/CI_ESRX',
        },
        'CVS_AET': {
            'train': './datasets/wtwt/o_CVS_AET',
            'test': './datasets/wtwt/CVS_AET',
        },
